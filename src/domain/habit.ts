...
export const needsNotificationUpdate = (
  ...
  return (
    ...
    newHabit.notification.message.trim() !==
      oldHabit.notification.message.trim() ||
...
export const calculateNextScheduleTime = (
  ...
  if (!validNotificationTime.isValid) {
    return {
      isValid: false,
      error: `Invalid notification time: ${validNotificationTime.error}`,
    };
  }
  ...
  if (!validCurrentTime.isValid) {
    return {
      isValid: false,
      error: `Invalid current time: ${validCurrentTime.error}`,
    };
  }
...
export const calculateMinutesUntilNotification = (
  ...
  const msUntilNotification =
    validNotificationTime.value!.getTime() - validCurrentTime.value!.getTime();
  return {
    isValid: true,
...
export const hasUnsavedChanges = (
  ...
  const currentDaysMatch =
    originalHabit.occurrence.type === "custom"
      ? JSON.stringify(currentDays) ===
        JSON.stringify((originalHabit.occurrence as CustomOccurrence).days)
      : true;
...
export const toggleDay = (currentDays: Days[], dayToToggle: Days): Days[] => {
